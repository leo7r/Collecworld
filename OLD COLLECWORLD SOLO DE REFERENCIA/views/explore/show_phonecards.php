...
<?php
	...
	case 'dated':
		$cat_name = $this->lang->line('tarjetas_fecha');
		break;
		
	case 'undated':
		$cat_name = $this->lang->line('tarjetas_sin_fecha');
		break;
		
	case 'not-emmited':
		$cat_name = $this->lang->line('tarjetas_uso_interno');
		break;
	...
	case 'magnetic-band':
		$sys_name = $this->lang->line('banda_magnetica');
		break;
	
	case 'optical':
		$sys_name = $this->lang->line('sistema_optico');
		break;
		
	case 'induced':
		$sys_name = $this->lang->line('sistema_inducido');
		break;
		
	case 'chip':
		$sys_name = $this->lang->line('chip');
		break;
		
	case 'remote-memory':
		$sys_name = $this->lang->line('memoria_remota');
		break;
		
	}
	$title_modal="<tr><td>category:</td><td>".$cat_name."</td></tr><tr><td>system:</td><td>".$sys_name."<td></tr><tr><td>country:</td><td>".$country."</td><tr><td>company:</td><td>".$company_name."<td></tr>";
...
<div id="content">
		...
		<div class="in">
			<div id="toolbar-left">
				<div class="item location">
					...
					<a href="<?php echo base_url().'index.php/explore/phonecard/?catalog='.$catalog; ?>"><?php echo $cat_name; ?></a>&nbsp;&raquo;&nbsp;
					<a href="<?php echo base_url().'index.php/explore/phonecard/?catalog='.$catalog.'&system='.$system; ?>"><?php echo $sys_name; ?></a>&nbsp;&raquo;&nbsp;
					<a href="<?php echo base_url().'index.php/explore/phonecard/?catalog='.$catalog.'&system='.$system.'&country='.$country; ?>"><?php echo ucfirst($country_name); ?></a>&nbsp;&raquo;&nbsp;
					<a href="<?php echo base_url().'index.php/explore/phonecard/?catalog='.$catalog.'&system='.$system.'&country='.$country.'&company='.$company; ?>"><?php echo ucfirst($company_name); ?></a>&nbsp;&raquo;&nbsp;
					
					<?php
					if ( $serie ){
					?>
						<a href="<?php echo base_url().'index.php/explore/phonecard/?catalog='.$catalog.'&system='.$system.'&country='.$country.'&company='.$company.'&serie='.$serie; ?>"><?php echo ucfirst($serie_name); ?></a>
					<?php
					}
					?>
				</div>
	...
	<?php
		$url = base_url().'index.php/explore/phonecard/'.$catalog.'/'.$system.'/'.$country.'/'.$company;
		if ( $serie )
			$url = $url.'/'.$serie;
		else
			$url = $url.'/0';
        	...
        	<tr>
            	<td>
                	<div id="explore_sort" class="box1">
						...
						<img  src="<?php echo base_url(); ?>img/own.png" title="<?php echo $this->lang->line('tengo_esta'); ?>"  id="buttons" class="fast_marquing" onclick="modalItemList(<?php echo count($phonecards); ?>,1,1,0,'<?php echo $title_modal; ?>')" />
