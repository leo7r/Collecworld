...
<script>
		...
		switch( num ){
			...
			case 3:
				...
				case 4:
					...
					if(clave1==clave2){
						info.innerHTML = '<?php echo $lang['contrasena_confirmada']; ?>.';
						info.className = 'reg_info1';
						b_4 = true;
					}else{
						info.innerHTML = '<?php echo $lang['confirmar'].' '.$lang['contrasena']; ?>.';
						info.className = 'reg_info0';
						b_4 = false;
					}
				}else{
	...
	function send(){
		...
		if ( b_0 && b_1 && b_2 && b_3 && b_4 && country != -1 && ( email == email2 ) ){ 
			...
			if ( email == email2 ){
				showGlobalInfo("<?php echo $lang['completa_todos_los_campos'] ?>");
			}
			else{
				showGlobalInfo("<?php echo $lang['correo_electronico_invalido'] ?>");
			}
		}
...
<div id="modal-content">
	<form id="signup_form" action="signup" method="post">
		<table id="reg_usr" cellspacing="15px">
			...
			<tr>
				<td><?php echo $lang['confirmar']." ".$lang['contrasena']; ?>: </td>
				<td><input class="input0" type="password" id="pass1" name="pass1" onkeyup="check('pass1',4);" onblur="check('pass1',4);" readonly /></td>
				<td class="reg_info"  id="pass1_i"><?php echo $lang['confirmar']." ".$lang['contrasena']; ?>!</td>
			</tr>
			<tr>
