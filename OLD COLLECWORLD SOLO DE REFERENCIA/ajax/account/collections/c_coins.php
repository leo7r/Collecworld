...
<div class="title4">
    <?php echo $lang['profile_mis_colecciones_monedas']; ?> [<?php echo $name_list; ?>]    
</div>
...
$countries_sql = 'SELECT coun.name , coun.id_countries , count(c.id_coins) as count FROM coins_users cu , coins c , countries coun WHERE cu.id_users = '.$_SESSION['id_users'].' AND cu.id_lists = '.$list;
$countries_sql = $countries_sql.' AND c.id_coins = cu.id_coins AND c.id_countries = coun.id_countries GROUP BY c.id_countries';
	...
	for ( $i = 0 ; $i < mysql_num_rows($countries_cursor) ; $i++ ){
        ...
        <td>
            <div class="collection-step2" 
            	onClick="coins_collections_select( <?php echo $_SESSION['id_users']; ?> ,
