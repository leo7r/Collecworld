...
if ( strpos($_SERVER['DOCUMENT_ROOT'],'wamp') == false ) {
	...
	if($_REQUEST['category']='phonecard'){
		...
		$sql_price2 = "SELECT p.id_users, p.id_phonecards, p.id_lists, p.status_phonecard, p.price, u.*, c.name as name_country, c.id_countries, SUM(tu.calification)/COUNT(tu.id_users) as cal, p.id_phonecards_users FROM phonecards_users p, countries c , users u LEFT JOIN trade_users tu ON u.id_users = tu.id_users WHERE p.id_phonecards = ".$_REQUEST['trade_article']." AND p.id_users=u.id_users AND u.id_countries=c.id_countries AND p.id_lists=".$num_type." AND u.id_users != ".$_SESSION['id_users']." GROUP BY p.id_phonecards_users ORDER BY p.price ASC";
		
		
		$sql_price = "SELECT p.id_users, p.id_phonecards, p.id_lists, p.status_phonecard, p.price, u.*, c.name as name_country, c.id_countries, SUM(tu.calification)/COUNT(tu.id_users) as cal, p.id_phonecards_users FROM phonecards_users p, countries c, users u LEFT JOIN trade_users tu ON u.id_users = tu.id_users WHERE p.id_phonecards=".$_REQUEST['trade_article']." AND p.id_users=u.id_users AND u.id_countries=c.id_countries AND p.id_lists=".$num_type." AND u.id_users != ".$_SESSION['id_users']." GROUP BY p.id_phonecards_users ORDER BY p.price ASC ";
		$cursor_price = mysql_query($sql_price);
		$num_price=mysql_num_rows($cursor_price);
...
<table id="trade-user-table" cellspacing="5">
    <tr>
        ...
        <td><?php echo $lang['ordenar_por']; ?>: 
            <select id="show_user_trade" onchange="show_user_trade(this);">
                ...
                <?php if ($_REQUEST['show']!=2){?><option value="2"><?php echo $lang['precio']; ?></option><?php } ?>
    ...
    <tr>
        <td colspan="2">
        <strong><?php echo $lang['ordenar_por_precio']; ?></strong>
        </td>
    </tr>
    <tr>
        <td colspan="2">
            ...
            if(!$num_price){
                ...
                while($datos_price=mysql_fetch_array($cursor_price)){
                    
            ?>
            
				<div id="user-trade" onClick="modalTradePhonecard(<?php echo $datos_price['id_phonecards_users']; ?>,<?php echo $num_type; ?>)" >
                    
                    <img id="user-image-trade" height="100" width="100" src="<?php echo $path.'users/img/'.$datos_price['image']; ?>"/>
                    <?php
					$cal = $datos_price['cal']*100;
                    ...
                    <div id="user-info-trade">
                    	<table border="0" width="100">
                       		<tr>
                            	<td><strong><?php echo $datos_price['user']; ?></strong> <span style="color:<?php echo $color; ?>"><?php echo $cal ?></span></td>
                            </tr> 
                            <tr>
                            	<td><?php echo $datos_price['name_country']; ?></td>
                            </tr>
                            <tr>
                            	<td>
								<?php 
								if($datos_price['status_phonecard']){
									
									echo $datos_price['status_phonecard'];
								}else{
                            ...
                            <tr>
                            	<td>
								<?php 
								if($datos_price['price']){
									
									echo '$ '.$datos_price['price'];
								}else{
									echo $lang['no_disponible'];	
								}
								?>
