...
$sql = 'SELECT c.name , c.id_countries FROM countries c , coins p , coins_users pu , users u WHERE p.id_coins = pu.id_coins AND c.id_countries = p.id_countries AND u.id_users = "'.$user.'" AND pu.id_users = u.id_users AND pu.id_lists = '.$list.' GROUP BY c.name';
...
for ($i=0 ; $i < $num ; $i++){
	...
	$sql2 = 'SELECT c.name , c.id_countries FROM countries c , coins p , coins_users pu , users u WHERE p.id_coins = pu.id_coins AND c.id_countries = p.id_countries AND u.id_users = "'.$user.'" AND c.id_countries = '.$datos['id_countries'].' AND pu.id_users = u.id_users AND pu.id_lists = '.$list;
