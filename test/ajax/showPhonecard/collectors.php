...
$sql = 'SELECT pu.* , u.name , u.user , u.image FROM phonecards_users pu , users u WHERE pu.id_users = u.id_users AND pu.id_phonecards = '.$id.' AND u.profile_privacy=0 AND id_lists = ';

if ( isset($_SESSION['id_users']) ){
	$sql_tail = ' AND pu.id_users <> '.$_SESSION['id_users'];
}
...
<div>
	<ul id="show-tabs">
		...
		<li class="box1 selected" onClick="showPhonecardTab(1,<?php echo $id; ?>);"><?php echo $lang['coleccionistas']; ?></li>
...
<div id="collectors-list">
	...
	if ( mysql_num_rows($collection) == 0 and mysql_num_rows($wish) == 0 and
	 mysql_num_rows($swap) == 0 and mysql_num_rows($buy) == 0 and mysql_num_rows($sell) == 0 ){
		?>
			<span class="title3"><?php echo $lang['no_coleccionistas_tarjetas_telefonicas']; ?>.</span>
		<?php
	 }
	 else{
