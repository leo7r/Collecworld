...
/** 
*
* acp_attachments.php [Argentinian Spanish]
*
* @package language
* @version $Id: $
...
$lang = array_merge($lang, array(
	'ACP_ATTACHMENT_SETTINGS_EXPLAIN'	=> 'Acá podes configurar los principales parámetros para archivos adjuntos y las categorías especiales asociadas.',
	'ACP_EXTENSION_GROUPS_EXPLAIN'		=> 'Acá podes añadir, borrar, modificar o deshabilitar los grupos de extensiones. Opciones adicionales incluyen su asignación a una categría especial, cambiar el mecanismo de descarga y definir un icono que se mostrará delante del adjunto que pertenezca al grupo.',
	'ACP_MANAGE_EXTENSIONS_EXPLAIN'		=> 'Acá podes editar sus extensiones permitidas. Para activar sus extensiones, por favor dirijite al Panel de Administración (ACP) de extensiones de grupo. te recomendamos encarecidamente no permitir extensiones de scripts (como <code>php</code>, <code>php3</code>, <code>php4</code>, <code>phtml</code>, <code>pl</code>, <code>cgi</code>, <code>py</code>, <code>rb</code>, <code>asp</code>, <code>aspx</code>, y similares).',
	'ACP_ORPHAN_ATTACHMENTS_EXPLAIN'	=> 'Acá podés ver qué archivos están en la carpeta de subidas sin ser asignados a ningún mensaje. Esto ocurre generalmente si el usuario adjunta un archivo pero no envía el mensaje. podés borrar estos archivos o asignarlos a un mensaje existente. Esto requiere de un ID de mensaje válido por lo que ha de determinarlo por su cuenta. Esta utilidad es principalmente para aquellas personas que quieren subir archivos (por lo general grandes) con otro programa (p.ej. con un cliente ftp) y asignarlos a un mensaje ya existente.',
	...
	'ATTACH_MAX_FILESIZE_EXPLAIN'		=> 'El tamaño máximo de cada archivo. Si el valor es 0, el tamaño del archivo cargable esta limitado por la configuración de PHP',
	...
	'DOWNLOAD_ADD_IPS_EXPLAIN'			=> 'Para especificar varias IPs o hostnames diferentes, introducí cada una en una nueva línea. Para especificar un rango de direcciones IP separá el inicio y el final con un guión (-), para especificar un comodín usá *',
	'DOWNLOAD_REMOVE_IPS_EXPLAIN'		=> 'Podes eliminar (o des-excluir) múltiples direcciones IP al mismo tiempo usando la combinación apropiada de ratón y teclado para tu navegador (p.ej. Ctrl+Clic). Las IPs <em>excluidas</em> están en negrita.',
	'DISPLAY_INLINED'					=> 'Mostrar imagenes',
	'DISPLAY_INLINED_EXPLAIN'			=> 'Si elegís "Sin imagen", los adjuntos se mostrarán como un enlace.',
	...
	'NO_WRITE_UPLOAD'					=> 'La carpeta de subidas que especificaste no se podés escribir. Por favor cambiá los permisos en el servidor.',
