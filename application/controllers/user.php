...
class User extends CW_Controller {
	...
	public function show_phonecard_collection( $user , $list_id , $id_countries , $id_companies , $system , $catalog , $no_variations = 0 , $lack = 0 ){
     ...
     pdf_create($html, $data['title'].$u['user'],true,0);		
	}
	public function show_phonecard_collection_xls( $user , $list_id , $id_countries , $id_companies , $system , $catalog , $no_variations = 0 , $lack = 0 ){
		...
		$query = $this->phonecard_model->get_user_collection( $u['id_users'] , $list_id , $id_countries , $id_companies , $system , $catalog , $no_variations , $lack );
		    ...
		    $title1[] = array ("Collecworld");

		    $title1[] = array ("");

		    $title1[] = array ($this->lang->line('informacion'));

		    $title1[] = array ($this->lang->line('usuario'), $u['user']);

		    $title1[] = array ( $this->lang->line('nombre'),$u['name']);

		    $title1[] = array ($this->lang->line('pais'),$u['Country']);

		     $title1[] = array ("");


		   $fields = ( $field_array[] = array ($this->lang->line('nombre'), $this->lang->line('tipo_sistema'), $this->lang->line('logo'),$this->lang->line('emitida'),$this->lang->line('tiraje'), $this->lang->line('valor_facial'),$this->lang->line('variacion_descriptiva')) );
		   ...
		   $xls->generateXML ( $data['title'].$u['user'] );	
